jQuery(document).ready(function () {
    ...
    jQuery('#hint-system').on('change', function () {
        ...
        if (selectedValue != "") {
            ...
            if (selectedValue == "APP-BASED HINTS") {
                var quote = app_based_hints.find(function (item) {
                ...
                var quote = live_moderators.find(function (item) {
                    // Check if the key in the object matches the head_countValue
                    // 
                    //console.log(Object.keys(item)[0]+'  '+head_countValue);
                    return Object.keys(item)[0] === head_countValue;
    ...
    jQuery(document).on('change', '#hint-system, #activity', function () {
    ...
    jQuery('#head_count').on('change', function () {
        ...
        if ($('#hint-system').length > 0) {
            var selectedValue = $('#hint-system').val();
            if (selectedValue != "") {
                var app_based_hints = [
                    { "Under 10": "$500" },
                    { "10-15": "$750" },
...
jQuery("body").on("click", "#email-my-quote-next", function () {
    ...
    if ($('#hint-system').length > 0) {
        ...
        var hint_system = $('#hint-system').val();
        if (hint_system == "") {
            jQuery('#hint-system').addClass("wpcf7-not-valid");
        } else {
            jQuery('#hint-system').removeClass("wpcf7-not-valid");
        }

        if (head_countValue != '' && hint_system != "") {
...
jQuery("body").on("click", "#email-to-quote-submit", function () {
    ...
    var hint_systemvalue = jQuery('#hint-system').val();
    ...
    console.log("email to quote fields value  FullName " + fullName + " email:" + companyEmail + "Phone Number " + phoneNumber + "head count: " + head_countValue + " hint_system  " + hint_systemvalue);
    ...
    if (fullName != '' && companyEmail != "" && pattern.test(companyEmail) && phoneNumber != "") {
        ...
        jQuery.ajax({
            ...
            type: 'POST',
            data: {
                action: 'instant_quote_form_submit2',
                head_count: head_countValue,
                currentpageurl: currentpageurl,
                hint_system: hint_systemvalue,
                full_name: fullName,
...
jQuery("body").on("click", "#check-date-time-next", function () {
    ...
    if (head_countValue != '' && hint_systemvalue != "") {
        jQuery('#form-step-1').removeClass('active');
        jQuery('#form-step-2').addClass('active');
...
jQuery("body").on("click", "#datetime-to-quote-submit", function () {
    ...
    console.log("Date to quote fields value  FullName " + fullName + " email:" + companyEmail + "Phone Number " + phoneNumber + "head count: " + head_countValue + " hint_system  " + hint_systemvalue + " date " + date + " timezone " + timezone + " time of day" + timeofday);
    if (fullName != '' && companyEmail != "" && pattern.test(companyEmail) && phoneNumber != "") {
        jQuery('#form-step-3').removeClass('active');
        ...
        jQuery.ajax({
            ...
            type: 'POST',
            data: {
                action: 'instant_date_form_submit2',
                head_count: head_countValue,
                hint_system: hint_systemvalue,
                full_name: fullName,
                CompanyEmail: companyEmail,
